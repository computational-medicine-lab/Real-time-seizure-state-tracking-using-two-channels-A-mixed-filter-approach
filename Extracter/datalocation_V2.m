...
%outputs info about the location of data, subject id, session id, number of
%sessions, and seizure timeing, this information will be used for splitting
%the training set(first seizure), validation set (second seizure), and test
%set (remaining), as well as defining the labels (seizure timings). If one
%session has more than one seizure, we will split that session into
%subsessions with one seizure later.
    fileID1=fopen('DownloadedData\RECORDS-WITH-SEIZURES','r');
    counter=1;
    while ~feof(fileID1) && counter < 51   
        ...
        fileID2=fopen(['DownloadedData\chb',num2str(Summary{counter,2},'%02.f'), '\chb' ,num2str(Summary{counter,2},'%02.f'), '-summary.txt'],'r');
